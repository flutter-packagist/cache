...
extension DataOption on BaseTableModel {
  ...
  * */
  String getSetContent(STSet obj) {
    if (obj.setList!.isEmpty ||
        obj.content == null ||
        obj.content!.isEmpty) return "";
    var value = "";
    for(var element in obj.content!){
      if (element >= 0 && element < obj.setList!.length) {
        if(value.isNotEmpty) value += ",";
        value += obj.setList![element];
      }
    }
    return value;
  }
  ...
  * 多条数据映射
  * */
  List<T> _mapData<T>(List<Map> data){
    List<T> list = [];
